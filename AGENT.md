...
- TUI/blessed code (`src/tui/**`, except pure helpers like `render.js`) has no automated coverage — verify changes there manually (`node src/cli.js demo` against the isolated `~/.mycelium-demo` store is the safe way to interact with realistic data without touching anything real).
