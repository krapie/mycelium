...
src/
  ...
  tui/                the neo-blessed interface — app.js (shell), views/ (sessions.js, calendar.js),
                       ...
                       tutorial.js + tutorial-data.js + tutorial-mock-llm.js (first-run tour / `mycelium demo`)
...
- **Human-facing text**: LLM prompts (in `learn.js`, `insight.js`, `organize.js`'s classification, `split.js`) and their expected JSON output are in Korean — this is a deliberate content-language choice, not something to translate. TUI strings go through `src/tui/i18n.js`'s `t()` (English default, Korean via `mycelium lang ko`). Code identifiers, comments, README/docs, and commit messages are English. The tutorial/`mycelium demo` is the one exception to the Korean-prompt rule: it never calls a real LLM (see Tests below), so its canned English output doesn't need to match the real prompts' language.
...
`node:test` (Node's built-in runner, zero new dependency) — `npm test` runs `test/*.test.js`. No mocking framework; hand-written fakes/seams only.
...
- **LLM-dependent modules** (`learn.js`, `insight.js`, `organize.js`'s classification, `split.js`'s boundary suggestion): `src/llm.js` exports `__setTestProvider(fn)`/`__clearTestProvider()` — an injection seam on `complete()`, used both by tests and (the one non-test caller) `tui/tutorial.js`'s `seedMockSessions()`/`endTutorial()`, via `tui/tutorial-mock-llm.js`'s `tutorialMockProvider()`, so the first-run tutorial/`mycelium demo` never spawns a real subprocess. Production path (real `claude`/`codex` subprocess) is untouched when no provider is set. Always `__clearTestProvider()` in `test.afterEach()`.
...
| Other | `Shift+M` merge · `Shift+S` split · `Shift+O` cycle sort · `y` copy · `d` digests · `g` re-show onboarding · `?` full shortcut list · `q` quit |
