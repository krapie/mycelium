...
src/
  ...
  tui/                the neo-blessed interface — app.js (shell), views/ (sessions.js, calendar.js),
                       ...
                       tutorial.js + tutorial-data.js (first-run tour / `mycelium demo`)
...
- **LLM-dependent modules** (`learn.js`, `insight.js`, `organize.js`'s classification, `split.js`'s boundary suggestion): `src/llm.js` exports `__setTestProvider(fn)`/`__clearTestProvider()` — a test-only injection seam on `complete()`. Production path (real `claude`/`codex` subprocess) is untouched when no test provider is set. Always `__clearTestProvider()` in `test.afterEach()`.
