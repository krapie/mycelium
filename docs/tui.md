...
Mycelium is keyboard-first, but every list responds to the mouse, and always the same way. **One click moves the cursor onto the row you clicked and gives that panel focus. A second click on the row already under the cursor does whatever `Enter` does there** (drill into a folder's sessions, open a session's detail, pick a menu entry, apply a checklist). **The wheel moves the cursor in whichever panel the pointer is over**, with the same live preview the arrow keys give, so the panels to the right follow along. Clicking a panel is also how you move focus between the three columns without arrowing back out.

That is the whole mouse surface, by design. Every action a click can reach is one the keyboard already reaches; the mouse never gets its own exclusive action. **Everything else is keyboard-only** — the action keys (`a`, `o`, `w`, `k`, `n`, `h`, `r`, `i`, `y`, `x`, `Shift+M`, `Shift+S`, ...), `Space` multi-select, `*` select-all, `p` preview inside a checklist, and `Esc` to back out or dismiss a modal (clicking outside a modal does **not** close it). In the **Calendar** tab, clicking the month grid focuses that panel, but picking a specific day is arrow keys only; the grid is drawn text, not a widget with clickable cells.

One tradeoff worth knowing: because mouse tracking is on, your terminal's own drag-to-select is suppressed. Hold `Shift` while dragging (`Option` on iTerm2) to bypass it and select text normally, then copy with `Cmd+C` / `Ctrl+Shift+C`. For a whole session, `y` is easier.
