...
**Scan step, and why `mycelium demo` can't just call the real `scan()` unguarded.** The tutorial's own mock sessions are seeded directly (`seedMockSessions()`'s `saveRaw()` calls), never discovered through a real `scan()` — but `scan()` itself reads each agent's REAL global store (`~/.claude`/`~/.codex`/`~/.kiro` — `adapters/index.js`), completely unaffected by `MYCELIUM_HOME`. Correct for the real store (including the first-run tutorial, which runs against it on purpose — that's the actual point of first-run scan), but wrong for `mycelium demo`'s isolated walkthrough: `MYCELIUM_HOME` already points at a throwaway `~/.mycelium-demo` specifically so nothing real gets touched, and pressing scan there (now an actual instructed step, not just something reachable by deviating from the script) would otherwise still pull real, potentially sensitive session content in regardless. `cli.js`'s `demo` command sets `MYCELIUM_DEMO_MODE=1` on the child process alongside `MYCELIUM_HOME`; `scan()` skips the real `ADAPTERS` loop entirely when it's set — scoped by an explicit flag, not by inspecting `MYCELIUM_HOME`'s path, so the first-run tutorial against the real store stays untouched. Net effect inside the demo: scan always reports `scanned 0, imported 0` — the step's own copy sets that expectation honestly (mechanics, not a fake dramatic import) rather than staging a whole fake-adapter injection just to make the count non-zero. [tested] (`test/scanner.test.js`'s `MYCELIUM_DEMO_MODE` case for the guard itself; the e2e Scan-step test above for the store's session count staying unchanged through a real demo run)
