...
- **Discover and import sessions.** `scan({ onImport })`. Skips excluded
  ...
  `summarizedTurnCount`) so capture never clobbers later-stage state. [partial]
...
- **Raw record CRUD.** `loadRaw`/`saveRaw`/`deleteRaw`/`allRaw` — `loadRaw`
  never throws (null on missing/corrupt file). [tested]
- **Look up a session by short id prefix.** `findSession(idOrPrefix)` —
  ...
  listed; zero matches → error. [tested]
...
- **List the full tree** (including empty folders). `listTreeDirs()`. `_inbox`
  excluded (virtual); `_archive` present but hidden from default views. [untested]
- **Rename/re-nest a folder.** `renameFolder(oldPath, newPath)`. Rejects
  ...
  **Special-cases case-only renames** on case-insensitive filesystems
  (macOS/Windows) via a temp-name two-step, to avoid deleting the source. [tested]
...
- **Delete a folder.** `deleteFolder(folderPath, { reassignTo })` — sessions
  reassigned (default `_inbox`), never orphaned. [tested]

## Organize — Manual session mutation (human-owned)

- **Move a session to a folder.** `move(sessionId, folderPath)` — sets
  `organizedBy: 'human'` (the sticky flag automation always respects). [tested]
...
- **Edit title/summary manually.** `setContent(sessionId, { title, summary })`
  ...
  again** so auto-tag refills it. Summary always refreshes regardless. [tested]
- **Delete a session (Mycelium-only, original log untouched).**
  ...
  backlinks; returns `touchedIds` for reindexing. [tested]
...
- **Link a handoff's parent/child sessions.** `linkContinuation(childId, parentId)`
  — no-op if `childId === parentId`; dedupes. [tested]
- **Fold a merge/split "product" into a real resumed session.**
  ...
  product (backlink sweep applies). [untested]
- **Merge sessions (git-like, reversible).** `mergeSessions(ids, {title})` /
  ...
  block gets a `role:'system'` separator turn. [tested]
...
- **Generate a daily/weekly narrative digest.** `generateDigest({period, date})`
  ...
  error before any LLM call; overwrites any existing digest for that key. [untested]
...
- **`CLAUDE.md` bridge, so Claude Code actually sees any of this.**
  ...
  Codex/Kiro/OpenCode users, and covers manual `i`-key inject, which doesn't know the
...
- **Propose + apply LLM topic-boundary splits.**
  ...
  is informational only, unlike merge's hiding via `supersededBy`). [partial]
...
- **Resolve the correct working directory to resume in.**
  ...
  path still exists on disk. [untested]
...
- **Claude Code adapter** — parses `~/.claude/projects/*/*.jsonl`,
  ...
  prose-only summaries (never raw payloads). [partial] (parse tested; corrupt-line
...
- **Kiro adapter** — unions 3 on-disk formats (SQLite v1/v2, JSONL
  ...
  [partial] **(only the JSONL fallback is tested — the SQLite v1/v2 paths, which
...
- **OpenCode adapter** — reads `~/.local/share/opencode/opencode.db`
  ...
  `session`/`message`/`part` tables, tool activity as prose-only summaries
  (never raw tool output/payloads). [partial] (`parse()` tested against a
  constructed SQLite fixture; `listSessions()`'s live default-DB-path
  resolution is untested, matching the same gap already noted for Kiro's
  SQLite paths)
...
- **Sqlite index rebuildable from raw files.** `reindex()` (full) /
  ...
  swallowed). [partial] (exercised transitively via `cleanup.test.js`, not directly)
...
- **Full-text + tag + folder search.** `search(...)` — FTS5, bm25-ranked
  ...
  quoted to survive punctuation/Korean text. [tested] (punctuation-safety asserted
...
- **Run prompts through the user's own CLI subscription** (no separate API
  ...
  SIGTERM on expiry. [partial] (the `__setTestProvider()` injection seam itself is
...
- **First-run onboarding prompt** — offers the interactive tutorial or a
  ...
  install and could show real `~/.claude`/`~/.codex`/`~/.kiro`/OpenCode session
...
**`.`-menu selections advance steps too, via a signal — not by guessing from Enter.** Selecting a palette item confirms via Enter, the same key that confirms/closes lots of other dialogs, so raw keypress-matching structurally can't tell "Enter selected Organize from the menu" apart from "Enter did something unrelated" — and polling `isModalOpen()` from step-entry doesn't work either, since opening the palette itself already flips the same screen-child-count heuristic used to detect the target modal, well before an item is even picked. `sessions.js`'s `doScan`/`doOrganize`/`doKnowledge`/`doMerge`/`doSplit` — the same named functions already shared between direct-key and menu dispatch — each call `app.tutorialSignal?.(name)` past their own guard checks (so e.g. Shift+M with fewer than 2 selected never fires a false "merge happened" signal), reported to whichever it is regardless of trigger path. `tutorial.js` consumes it with the same lifecycle as `app.quitGuard` (set at tutorial start, cleared in `finish()`), and shares the exact same advance-and-poll tail as a matched keypress (`advanceFrom()`) — a menu-triggered organize/knowledge/merge/split still waits for the real review modal to open before advancing, same as the direct-key path; only Scan (no review modal at all) advances on the signal itself, fired at genuine completion (`scan.done`), not function-entry. [tested] (`test/e2e/demo-e2e.test.js`'s "selecting Organize from the \`.\` action menu advances the narrator too" and "the new Scan step advances via the \`.\` menu too" cases drive both real menu-selection paths end to end)

**Scan step: the Sessions view starts genuinely empty, and pressing `s` is what actually fills it in.** `index.js`'s real tutorial-launch paths (`mycelium demo` and first-run onboarding) mount the Sessions view empty and call `prepareTutorialProvider(personaId)` only (LLM mock + a pre-staged knowledge proposal — no session rows). The mock sessions themselves are written by `injectDemoSessions(personaId)` (`tutorial.js`), which fires from `app.tutorialSignal('scan')` — the same signal `sessions.js`'s `doScan()` sends on genuine completion — so "press `s` to capture them" is literally true, not staged: nothing is on screen until that exact keypress, then the persona's full mock set appears via the caller-supplied `reloadSessions` callback. `seedMockSessions()` still exists as a convenience wrapper (both steps, eagerly) for callers that want the whole demo store ready without driving the tutorial through its own Scan step — most of the e2e suite still uses it, passing `sessionsPreSeeded: true` to `startTutorial()` so the Scan step's own injection is skipped for them (`injectDemoSessions()` is not idempotent — `buildMockSessions()` stamps a fresh `randomUUID()` per call — so injecting on top of an eager pre-seed would double the set; guarding is an explicit flag from the caller, not a store-state check, since "does a demo session exist anywhere" is unreliable both against this suite's shared, uncleaned test store and against a real store carrying a stale leftover from an uncleanly-exited prior tutorial). Separately: `scan()` itself reads each agent's real global store (`~/.claude`/`~/.codex`/`~/.kiro`/OpenCode's own `opencode.db`), completely unaffected by `MYCELIUM_HOME` — a problem for both tutorial-launch paths, not just `mycelium demo`'s isolated walkthrough: first-run onboarding runs against the real `~/.mycelium`, so a real, unguarded `scan()` there would import the user's actual real content into the very same batch `injectDemoSessions()` is about to add, both landing in one indistinguishable reveal. `startTutorial()` (`tutorial.js`) sets `MYCELIUM_DEMO_MODE='1'` itself for the tutorial's whole lifetime, saving/restoring whatever value was there before (so `mycelium demo`'s own child-process `MYCELIUM_DEMO_MODE=1` isn't disturbed either) — `scan()` skips the real `ADAPTERS` loop entirely while it's set, so the real scan it also runs alongside the mock injection always reports `scanned 0, imported 0` for as long as either tutorial flow is active. The user's actual first real scan still happens — just once onboarding concludes and `startUpkeepAndRecheck()`'s background `scanCycle()` takes over (`index.js`), not interleaved with the demo walkthrough itself. [tested] (`test/scanner.test.js`'s `MYCELIUM_DEMO_MODE` case for the scanner-side guard; `test/e2e/demo-e2e.test.js`'s "the Sessions view is genuinely empty until the Scan step" case for the empty-then-inject behavior end to end, "the new Scan step advances via the `.` menu too" for the already-pre-seeded/no-duplication path, and the two `startTutorial() sets/leaves MYCELIUM_DEMO_MODE` cases for the set-during/restore-after lifecycle toggle, both the normal unset-before case and `mycelium demo`'s own pre-set-to-'1' case)
