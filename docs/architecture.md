...
Everything is stored locally under `~/.mycelium/`. **Files are the source of
truth; sqlite is a derived index** (rebuild it any time with
`mycelium reindex` if it's ever deleted):
...
- **Local-only**: sessions contain sensitive work, so nothing leaves the
  machine. The interface is a local terminal TUI; LLM calls go through your
  own CLI subscription.
- **Model-agnostic**: the storage format is a neutral schema, not any one
  vendor's session format. Adding a new agent = one adapter file (see
  [`CONTRIBUTING.md`](../CONTRIBUTING.md)).
- **Human-first**: automatic filing/tagging are only suggestions —
