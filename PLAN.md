...
**참조**:
- **pi-session-manager** — 계층 태그의 구현 선례가 가장 구체적: sqlite `parent_id` 컬럼, `getDescendantIds()` 재귀 조회, 태그+텍스트 결합 SearchFilterBar, 하위 태그 포함 필터링(descendant filtering). 스키마와 쿼리 패턴을 그대로 참고.
...
- **Web UI**: 빌드 스텝 없는 단일 페이지 (폴더 트리 + 세션 리스트 + 태그 필터 + 검색 + 다이제스트 뷰). 복잡도가 낮으면 vanilla, 뷰가 4개를 넘으면 Preact+htm CDN import
- **CLI**: `mycelium scan | search | digest | context | handoff | daemon`
...
| 3 | 폴더 트리 + 태그 스키마 + cwd 기반 자동 배치 | 세션이 폴더에 배치되고 태그 필터+검색 결합이 동작 |
...
2. 자동 태깅 결과를 훑어보고 오분류율을 체감 수준에서 평가 — 태그 어휘가 발산하는지 확인
