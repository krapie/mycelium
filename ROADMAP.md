...
- **No separate API key or credential.** `complete()` rides the user's existing `claude` / `codex` (and, later, `kiro` / `opencode`) subscription. Anything that would require a dedicated paid key gets flagged as a principle deviation and decided explicitly, not adopted silently ([#86](https://github.com/krapie/mycelium/issues/86)).
