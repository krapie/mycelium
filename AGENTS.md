...
- **Fixes line**: if the PR resolves a tracked GitHub issue, include `Fixes #<issue number>` on its own line (matching `.github/PULL_REQUEST_TEMPLATE.md`'s own working syntax — no colon; GitHub only auto-closes the linked issue on merge for this exact keyword form). Omit entirely when there's no tracked issue — most bug reports/requests in this repo arrive directly in conversation, not as an issue first.
