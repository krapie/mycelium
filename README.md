...
4. **Learn**
   - `a` — 세션을 요약·태깅합니다. 손질 규칙을 결정(decision)으로 뽑아
     ...
     자동 추출이 부실하면 `e`로 직접 다듬어 적어도 됩니다(이후 `a`를 다시
     돌려도 이 편집은 보존됩니다 — 위 세션 패널의 `e` 항목 참고).
...
다시 쓰는 것보다 안전합니다 — 잘못된 요약이 검증 없이 다음 케이스로 그대로
주입되는 걸 막아줍니다. 데몬의 스캔 주기에 `w`까지 자동으로 끼워 넣는 것도
가능하니, 원하면 요청하세요.
