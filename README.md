...
맨 위 **`All`**은 전체 세션(고정, 이름변경/이동/삭제 불가)이고, 사용자가 만든 실제 폴더들은 그 아래 한 단 들여써서 표시됩니다. `_inbox` 같은 별도 폴더는 없습니다 — 아직 어느 폴더에도 배정되지 않은 세션은 `All`에서 목록에 그대로 보이고, 제목/요약 옆에 **`[New]`** 표시가 붙습니다. `m`으로 원하는 폴더로 옮기면 표시가 사라집니다.
...
**`_archive` 폴더는 TUI에 기본적으로 안 보입니다** — 폴더 목록, `All` 세션 뷰, 검색 결과 어디에도 안 뜹니다. 죽은 cwd 세션이 자동 정리 시(`autoOrganize`/`mycelium organize`) 여기로 모이고, `m`으로 아무 세션이나 `_archive`에 직접 옮겨도 됩니다 — 눈에 안 띄는 보관함입니다. 데이터는 그대로 있고 지워지지 않으니, 나중에 확인하려면:
```sh
